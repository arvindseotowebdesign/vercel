...
export const AddAdminProduct = async (req, res) => {
  try {
    ...
    const newProduct = new productModel({
      ...
      tag,
      features,
      specifications: updatespecifications, gst, weight
    });
...
export const exportAllProAdmin = async (req, res) => {
  ...
  try {
    ...
    const products = await productModel.find({}, 'title description pImage  images slug regularPrice salePrice status stock weight').lean();
    ...
    try {
      ...
      for (const productData of jsonData) {
        if (productData.proid) {
          console.log('productData._id', productData.proid.replace(/"/g, ''))
          // Check if the product already exists
          let productDataid = productData.proid.replace(/"/g, '')
          const existingProduct = await productModel.findById(productDataid);

          if (existingProduct) {
            // Update existing product
            await productModel.findByIdAndUpdate(productDataid, productData);
